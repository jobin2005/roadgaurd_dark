...
export function renderProfilePage(container) {
    ...
    content.style.cssText =
        "flex:1;padding:2rem 1rem;max-width:1000px;margin:0 auto;width:100%;";
...
function displayProfile(user, profile) {
  ...
  `;
    ...
    profileCard
        ...
        .addEventListener("submit", (e) => handleProfileUpdate(e, user.id));
...
async function displayContributions(user) {
    try {
        ...
        const active =
            potholes?.filter((p) => !p.status || p.status === "active")
                .length || 0;
        const removed =
            potholes?.filter((p) => p.status === "removed").length || 0;
        const highCount =
        ...
        statsGrid.appendChild(
            createStatBox("Active", active, "var(--success)"),
        );
        statsGrid.appendChild(
            createStatBox("High Severity", highCount, "var(--error)"),
        );
        ...
        if (!potholes || potholes.length === 0) {
            ...
            emptyMsg.textContent =
                "No reports yet. Start by uploading pothole images!";
            emptyMsg.style.color = "var(--text-secondary)";
            statsCard.appendChild(emptyMsg);
        } else {
            // Show ALL reports (no limit)
            const reportsContainer = document.createElement("div");
            reportsContainer.style.cssText =
                "max-height: 520px; overflow-y: auto; display:flex;flex-direction:column;gap:0.5rem;padding-right:0.25rem;";
...
function createReportItem(pothole, index) {
  ...
  `;
    reportItem.onmouseenter = () => {
        reportItem.style.borderColor = "var(--accent-primary)";
    };
    reportItem.onmouseleave = () => {
        reportItem.style.borderColor = "transparent";
    };
...
function createStatBox(label, value, color) {
  ...
  `;
    ...
    lbl.style.cssText =
        "margin:0;color:var(--text-secondary);font-size:0.82rem;";
